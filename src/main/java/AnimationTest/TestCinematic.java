...
public class TestCinematic extends SimpleApplication {
    ...
    public void simpleInitApp() {
        ...
        NiftyJmeDisplay niftyDisplay = new NiftyJmeDisplay(getAssetManager(), getInputManager(), getAudioRenderer(), guiViewPort);//, colorSpace
        ...
        // 1.24-second bone animation loop begins at t=2 seconds一起使用。俏皮将使用纹理图集来渲染。
        // 你通过Nifty渲染的每个图形资产将被放置到纹理图集中。
        // 我们的目标是在一个(或至少很少的)draw调用中渲染所有的俏皮组件。这将大大加快渲染速度。这个调用将为俏皮使用默认的
